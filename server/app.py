# server/app.py — v0.8.0  (Only Whole-Sign / Rāśi houses; JHora-style)
import os, sys
...
# Lahiri sidereal, mean node  — enako kot v JHori
...
class BirthData(BaseModel):
    ...
    hour: int; minute: int
    lat: float; lon: float
    tz: Optional[float] = None   # če ni podan, se izračuna sam
...
def ascendant_sid(jd_ut: float, lat: float, lon: float) -> float:
    # vzemi sidereal ascendent iz SwissEphemerisa
...
def chart_full(data: BirthData):
    jd = julday_ut(data)
    pl = planets_sid(jd)
    asc = ascendant_sid(jd, data.lat, data.lon)

    # WHOLE-SIGN (Rāśi) hiše:
    lagna_sign_idx = int(asc // 30)   # 0..11
    # začetne stopinje posameznih hiš (na začetku znaka)
    whole_cusps = [((lagna_sign_idx + i) % 12) * 30.0 for i in range(12)]

    def bhava_whole(lon: float) -> int:
        s = int(lon // 30)                 # znak planeta
        d = (s - lagna_sign_idx) % 12      # razdalja v znakih od lagna znaka
        return d + 1

    planets = {
        p: {"deg": round(lon, 2), "sign": sign_of(lon), "bhava": bhava_whole(lon)}
    ...
    return {
        ...
        "house_cusps_whole": {f"House{i+1}": round(whole_cusps[i], 2) for i in range(12)},
        "planets": planets,
