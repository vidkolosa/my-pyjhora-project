# server/app.py — v0.9.1 (Whole-Sign; Lahiri; Mean node; JHora alignment via env AYANAMSHA_CORR_DEG)
import os, sys, re
...
# Lahiri sidereal; Mean node — JHora-style base
swe.set_sid_mode(swe.SIDM_LAHIRI, 0, 0)
FLAGS_SID = swe.FLG_SWIEPH | swe.FLG_SPEED | swe.FLG_SIDEREAL

# ----- JHora alignment (IMPORTANT) -----
# Set this in Render ENV to match Jagannath Hora exactly:
# AYANAMSHA_CORR_DEG = -0.8867   # NEGATIVE value
AYAN_CORR = float(os.environ.get("AYANAMSHA_CORR_DEG", "0.0"))

def _apply_corr(lon: float) -> float:
    x = (lon + AYAN_CORR) % 360.0
...
def ascendant_sid(jd_ut: float, lat: float, lon: float) -> float:
    # Placidus for ASC, then apply sid+ayan correction
    asc = swe.houses_ex(jd_ut, lat, lon, b'P', FLAGS_SID)[1][0]
    return _apply_corr(norm(asc))
...
def planets_sid(jd_ut: float) -> Dict[str, float]:
    ...
    out = {k: _apply_corr(norm(swe.calc_ut(jd_ut, pid, FLAGS_SID)[0][0])) for k, pid in ids.items()}
...
def _compute(bd: BirthData):
    ...
    planets = {
        p: {"deg": round(pl[p], 5), "sign": sign_of(pl[p]), "bhava": bhava_whole(pl[p], lagna_sign_idx)}
        for p in pl
    }
    return {
        "ayanamsa": "Lahiri",
        ...
        "ascendant": {"deg": round(asc, 5), "sign": sign_of(asc)},
        "house_cusps_whole": {f"House{i+1}": round(cusps_whole[i], 2) for i in range(12)},
        "planets": planets,
        "chara_karakas": chara_karakas({k: planets[k]["deg"] for k in planets})
    }
